...
void test_check_address(void** state) {
    ...
    /* The public key is hardcoded. Indeed, get_public_key is not available from
     * tests because of exceptions and cryptographic calls. */
    cx_ecfp_public_key_t public_key = {
        .curve = CX_CURVE_256K1,
        .W_len = 33,
        .W = "\x02\x0f\x14\x58\x1f\x0e\xab\x34\xe5\xc9\x58\xbf\xa0\x40\x1b\x0f"
             "\x26\x5c\x16\x61\x70"
             "\xac"
    ...
    assert_int_equal(parse_bip32_path(bip32_path_ptr, bip32_path_length,
                                      bip32_path_parsed, MAX_BIP32_PATH),
                     1);
...
void test_get_printable_amount(void** state) {
    ...
    uint64_t amount;
    assert_int_equal(
        swap_str_to_u64(params.amount, params.amount_length, &amount), 1);
    assert_int_equal(amount, 0x4d2);
    assert_int_equal(xah_print_amount(amount, params.printable_amount,
