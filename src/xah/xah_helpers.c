...
static size_t xah_encode_base58_address(const base58_buf_t* in,
                                        xah_address_t* out) {
...
size_t xah_public_key_to_encoded_base58(xah_pubkey_t* pubkey,
                                        ...
                                        xah_address_t* out, uint16_t version) {
    base58_buf_t tmp;
    ...
    if (pubkey != NULL) {
        error = xah_public_key_hash160(pubkey, tmp.buf + version_size);
    ...
    return xah_encode_base58_address(&tmp, out);
}

void xah_compress_public_key(cx_ecfp_public_key_t* public_key,
                             xah_pubkey_t* out) {
...
bool parse_bip32_path(uint8_t* path, size_t path_length, uint32_t* path_parsed,
...
void get_address(cx_ecfp_public_key_t* pubkey, xah_address_t* address) {
    /* sizeof(xah_pubkey_t) < sizeof(xah_address_t) */
    xah_pubkey_t* p = (xah_pubkey_t*)address;
    xah_compress_public_key(pubkey, p);

    uint8_t addr_len = xah_public_key_to_encoded_base58(p, NULL, address, 0);
