...
err_t read_amount(parseContext_t* context, field_t* field) {
    ...
    if ((first_byte >> 7u) == 0) {
        CHECK(read_fixed_size_field(context, field, XAH_AMOUNT_LEN));
    } else {
        ...
        if (has_non_standard_currency(field)) {
            ...
            currency->id = XAH_CURRENCY_CURRENCY;
            currency->data.currency = (xah_currency_t*)(field->data.ptr + 8);
            currency->length = XAH_CURRENCY_SIZE;
        }

        field_t* issuer;
        CHECK(append_new_field(context, &issuer));
        issuer->data_type = STI_ACCOUNT;
        issuer->id = XAH_ACCOUNT_ISSUER;
...
err_t read_vector256_field(parseContext_t* context, field_t* field) {
    ...
    CHECK(read_fixed_size_field(context, field, XAH_VECTOR256_SIZE * count));
    for (size_t i = 0; i < count; i++) {
        ...
        hash256->data.hash256 =
            (hash256_t*)(field->data.ptr + (i * XAH_VECTOR256_SIZE));
...
err_t handle_path_step(parseContext_t* context, field_t* field,
    ...
    switch (step_type) {
        case 0x01:
            ...
            field->id = XAH_ACCOUNT_ACCOUNT;
            break;
        case 0x20:
            field->data_type = STI_ACCOUNT;
            field->id = XAH_ACCOUNT_ISSUER;
            break;
        case 0x30:
        ...
        case 0x10:
            ...
            field->id = XAH_CURRENCY_CURRENCY;
            break;
        default:
...
err_t post_process_field(parseContext_t* context, field_t* field) {
    ...
    switch (field->data_type) {
        ...
        case STI_VL:
            ...
            if (field->id == XAH_VL_SIGNING_PUB_KEY && field->length == 0) {
        ...
        case STI_ACCOUNT:
            ...
            if (field->length != XAH_ACCOUNT_SIZE) {
                err.err = INVALID_STATE;
                return err;
            }
            break;
...
err_t post_process_transaction(parseContext_t* context) {
    ...
    if (context->transaction_type == TRANSACTION_SET_REGULAR_KEY &&
        ...
        field->data_type = STI_ACCOUNT;
        field->id = XAH_ACCOUNT_REGULAR_KEY;
